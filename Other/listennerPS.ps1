...
while ($http.IsListening) {
    ...
    if ($context.Request.HttpMethod -eq 'POST' -and $context.Request.RawUrl -eq '/post') {
        ...
        $releases = ConvertFrom-Json $FormContent
        
        write-host $releases.item
        write-host $releases.value
        write-host $releases.read

        WiserKNX -idgroup $releases.item -value $releases.value
...
# Note:
# To end the loop you have to kill the powershell terminal or http://192.168.10.100:8080/kill. ctrl-c wont work :/
