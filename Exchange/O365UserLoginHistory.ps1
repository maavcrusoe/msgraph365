...
$Days = Read-Host -Prompt "Please enter how many days to export"
...
if (!(Test-Path -Path $PSScriptRoot\export)){ mkdir $PSScriptRoot\export }

$OutputCSV="$PSScriptRoot\export\AuditLog-$UserName_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" 
...
else
{
 if((Test-Path -Path $OutputCSV) -eq "True") 
 {
  ...
  Write-Host `n$OutputCSV  
  explorer /select,$OutputCSV
 }
